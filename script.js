...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // RSVP Form Handler (Form 2 – Dynamic RSVP)
...
function initFloatingMusicPlayer() {
    ...
    function toggleFloatingMusic() {
        if (isPlaying) {
            ...
            isPlaying = false;
            musicPlaying = false; // Update global variable
        } else {
            backgroundMusic.play().then(() => {
                ...
                isPlaying = true;
                musicPlaying = true; // Update global variable
            }).catch(error => {
    ...
    // Simpan state musik di localStorage (opsional)
    ...
    if (wasPlaying) {
        // Auto-play mungkin diblokir browser, jadi kita set icon saja
        if (playIcon) playIcon.style.display = 'none';
        if (pauseIcon) pauseIcon.style.display = 'block';
        musicPlayer.classList.add('playing');
        isPlaying = true;
...
// Music Controls (Existing functions - tetap dipertahankan untuk kompatibilitas)
